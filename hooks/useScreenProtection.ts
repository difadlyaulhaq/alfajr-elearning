...
export const useScreenProtection = (options: ScreenProtectionOptions = {}) => {
  ...
  // Optimized black screen trigger - reusable function
  const triggerBlackScreen = () => {
    ...
    blackScreenTimeoutRef.current = setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const cleanupRecording = detectScreenRecording();
    ...
    return () => {
      ...
      if (cleanupRecording) cleanupRecording.then(fn => fn && fn());
