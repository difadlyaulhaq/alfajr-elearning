...
import { LayoutDashboard, Compass, BookCopy, Award, LifeBuoy, LogOut, ChevronLeft, Loader2, Sparkles, TrendingUp } from 'lucide-react';
...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col w-64 bg-gradient-to-b from-black via-gray-900 to-black text-gray-100 border-r border-gray-800/30 h-screen sticky top-0 shadow-2xl">
      ...
      <div className="relative p-6 border-b border-gray-800/50">
        ...
        <Link href="/learning/dashboard" className="relative block">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-br from-[#C5A059] to-[#8B7355] rounded-xl blur-lg opacity-30"></div>
            <div className="relative bg-white/95 backdrop-blur-sm rounded-xl p-3 shadow-xl border border-[#C5A059]/30">
              <Image 
                src="/logo-alfajr.png" 
          ...
          <div className="mt-3 text-center">
            <div className="flex items-center justify-center gap-1.5">
              ...
              <p className="text-xs font-bold bg-gradient-to-r from-[#C5A059] to-[#D4AF37] bg-clip-text text-transparent">
                Learning Portal
              </p>
            </div>
          </div>
      ...
      {user && (
        <div className="px-4 py-4">
          <div className="bg-gradient-to-br from-gray-800/40 to-black/40 backdrop-blur-sm rounded-xl p-4 border border-gray-700/30 shadow-lg">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-br from-[#C5A059] to-[#8B7355] rounded-full blur-md opacity-50"></div>
                <div className="relative w-12 h-12 bg-gradient-to-br from-[#C5A059] to-[#8B7355] rounded-full flex items-center justify-center font-bold text-white shadow-xl border-2 border-white/20">
                  {user.name.charAt(0).toUpperCase()}
                </div>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-white truncate">{user.name}</p>
                <p className="text-xs text-[#C5A059] truncate">{user.division}</p>
              </div>
            </div>
      ...
      {/* Navigation Menu */}
      <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-black">
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all relative overflow-hidden ${
                isCurrentPath
      ...
      {/* Bottom Section */}
      <div className="px-4 py-4 border-t border-gray-800/50 space-y-2 relative">
        ...
        {user && user.role === 'admin' && (
          <Link 
            ...
            className="relative flex items-center px-4 py-2.5 text-sm font-medium rounded-xl text-gray-300 hover:bg-gradient-to-r hover:from-gray-800/30 hover:to-gray-700/30 hover:text-[#C5A059] transition-all group border border-gray-800/30 hover:border-[#C5A059]/30"
          >
